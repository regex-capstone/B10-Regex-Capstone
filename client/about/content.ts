export const ABOUT_JSON = {
    hero: {
        title: 'Meet ISAAC, the ultimate academic advising sidekick!',
        subtitle: 'With ISAAC, advisers can effortlessly answer student questions without needing to schedule an appointment and take academic advising to the next level.',
        button: 'CHECK IT OUT'
    },
    why: {
        title: 'Why Should You Choose ISAAC?',
        subtitle_1: 'We have upgraded upon the current experience for the advisers to easily maintain and distribute information to students, and for the students to consume information in a quick, searchable manner.',
        features: [
            {
                icon: '/img/search-engine.png',
                text: 'Searchable index to allow students to query for their answers.',
                attribution: '<a href="https://www.flaticon.com/free-icons/web-portal" title="web portal icons">Web portal icons created by Vectors Tank - Flaticon</a>'
            },
            {
                icon: '/img/ssg.png',
                text: 'Static-Site Generation for SEO and faster content fetching.',
                attribution: '<a href="https://www.flaticon.com/free-icons/gear" title="gear icons">Gear icons created by DinosoftLabs - Flaticon</a>'
            },
            {
                icon: '/img/dashboard.png',
                text: 'Data analytic dashboards to support data-driven decisions for advisers.',
                attribution: '<a href="https://www.flaticon.com/free-icons/dashboard" title="dashboard icons">Dashboard icons created by Eucalyp - Flaticon</a>'
            },
            {
                icon: '/img/text-editor.png',
                text: 'Rich text editor to allow advisers to easily create and edit content.',
                attribution: '<a href="https://www.flaticon.com/free-icons/text-editor" title="text editor icons">Text editor icons created by Noor Hakim - Flaticon</a>'
            }
        ]
    },
    who: {
        title: 'Who are We?',
        subtitle: 'We’re built by our history as Informatics students who have asked many questions to the iSchool Advising Team.',
        team: [
            {
                name: 'Elbert Cheng',
                roles: ['INFO Student', 'Project Director'],
                picture: 'https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_1280.png'
            },
            {
                name: 'Alan Wen',
                roles: ['INFO Student', 'Project Engineer'],
                picture: 'https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_1280.png'
            },
            {
                name: 'Keith Ellingwood',
                roles: ['INFO Student', 'Data Engineer'],
                picture: 'https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_1280.png'
            },
            {
                name: 'Ryan Langford',
                roles: ['INFO Student', 'Front-end Engineer'],
                picture: 'https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_1280.png'
            },
            {
                name: 'Brian Park',
                roles: ['INFO Student', 'Front-end Engineer'],
                picture: 'https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_1280.png'
            }
        ],}
}